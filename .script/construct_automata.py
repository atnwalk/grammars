...
def main(grammar, limit):
    ...
    with open(filename + '_automata.json', 'w+') as fd:
        json.dump(transformed, fd, indent=4)
...
def tokenize(rule):
    ...
    # 'string'
    if rule.startswith("'"):
        # TODO: this may fail but should be fine for the moment (index out of bounds possible if invalid)
        offset = 1
        end = -1
        while True:
            end = rule[offset:].index("'")
            offset += end + 1
            count = 0
